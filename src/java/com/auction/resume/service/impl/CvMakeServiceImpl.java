...
public class CvMakeServiceImpl implements CvMakeService{
	...
	public String addIntent(final HttpServletRequest request) {
    	...
    	try {
        	...
        	if(request.getParameter("expectedSalary")!=null && (Util.isVerify(request.getParameter("expectedSalary"),0,0,"[1-9][0-9]{0,1}")
        			|| request.getParameter("expectedSalary").equals("0.001"))){
        		expectedSalary = request.getParameter("expectedSalary");
        	}
        	if(jobTitle!=null && expectedSalary!=null){
                ...
                if(flag>0 && Util.isVerify(request.getParameter("content"),2,100,null)){
                	...
                	final String[] email = content.split(",");
	...
	public String addEducations(HttpServletRequest request) {
		...
		try {
			...
			if(Util.isVerify(request.getParameter("startTime"),0,0,"[1-2][0-9]{3}/[0-9]{2}")){
				paramsMap.put("startTime", request.getParameter("startTime"));
			}
			if(Util.isVerify(request.getParameter("endTime"),0,0,"[1-2][0-9]{3}/[0-9]{2}")){
	...
	public String addWorkExperiences(HttpServletRequest request) {
		...
		try {
			...
			if(Util.isVerify(request.getParameter("workContent"), 2,800,null)){
				paramsMap.put("workContent", request.getParameter("workContent").trim());
			}
			
			int flag = -1;
			if(Util.isVerify(request.getParameter("id"), 0, 0, "[0-9]{1,10}")){
	...
	public String addProjects(HttpServletRequest request) {
		...
		try {
			...
			if(Util.isVerify(request.getParameter("projectDetails"), 2, 1000,null)){
