...
function($, angular, app, cookie, url) {
	app.controller('vipCtrl', ['$scope', '$rootScope', '$http', '$q', 'uiservice',
		...
		$scope.showVipPool = function(){
		 ...
		 $('#prica').click(function(){
			  $('html,body').animate({scrollTop:$('#pricmd').offset().top}, 700);
			 });
		/* onclick="javascript:document.getElementById('footer').scrollIntoView()"*/
		/* 跳转支付*/
		$scope.jumpPay = function(type) {
		    ...
		    try {
				...
				if(result.code==200) {
			    	...
			    	uiservice.notify('您好，50个企业名额已满，如有最新信息我们会第一时间通知您，谢谢！', result.code != 200);
			    }
			} catch (e) {
