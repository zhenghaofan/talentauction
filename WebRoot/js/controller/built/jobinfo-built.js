...
define("js/common/almond", function(){});
...
define('app',['$', "angular", 'cookie'], function($, angular, cookie) {
...
define('upload',[],function() {
	...
	exports.fileUpload = function(config, callback, errorCallback) {
		jQuery.ajaxFileUpload({  
			url: config.url,
			secureuri: false,
			fileElementId: config.id,
			dataType: 'xml',
...
define('uiservice',['$','angular','app'], function($, angular, app) {
...
;(function($) {
    ...
    $.Zebra_DatePicker = function(element, options) {
        ...
        var init = function(update) {
            ...
            datepicker = $(html).addClass(options.text);
...
define("zebra_datepicker", function(){});
...
function($, angular, app, cookie, url, upload) {
	app.controller('jobinfoCtrl', ['$scope', '$rootScope', '$http', '$q', 'uiservice', 'info',
		...
		$scope.typeSlt = function(status) {
			...
			$scope.fileName = '';
			if(status == 0) $scope.getProgress();
			$scope._checkBrowser(status);
		};
		
		/* 检查浏览器*/
		$scope._checkBrowser = function(status) {
		...
		$scope.fileUpload = function() {
			...
			upload.fileUpload(config, function(result) {
				var data = result;
				if(data.code == 200) {
					window.location.replace('/talentform/jobresult');
				}else {
		...
		$scope.saveUrl = function() {
			...
			var url = $scope.urlStatus == 2? '/cvmake/updateProductUrl' : '/cvmake/addProductUrl';
			
			if(!_checkUrlInfo(config)) return;
			
            $q.when($http.post(url, $.param(config)))
            .then(function(result) {
            	var data = result.data;
            	if(data.code == 200) {
            		$scope.getResume();
		...
		/*打开翻页*/
		$scope.editSummodal = function() {
			$scope.show('.sum-modal');
		};
		/* 保存个人总结*/
		$scope.saveSum = function() {
			if($scope.sumInfo.text.length<2 || $scope.sumInfo.text.length>800) {
				$('#sum_name').focus();
				$('#sum_name_tip').show().text('个人总结长度需要大于2！');
				return ;
			}
			var config = {
			...
			if(!$scope.isIe) {
		    	...
		    	for(i in e.files) {
		    		...
		    		if(typeof e.files[i] == 'object') {
		    			if( e.files[i].size >= 3 * 1024 *1024 ) {
		    				flag = false;
		    				uiservice.notify('图片过大！限制3M', true);
		    	...
		    	if ($scope.imgName.indexOf('.jpg') == -1 && $scope.imgName.indexOf('.gif') == -1 && 
		    		...
		    		$('#img_name_tip').show().text('非图片类型文件，请重传！');
	            	$('input[name="upload"]').val('');
	                return false;  
	            }
		    }
		...
		function _checkFile () {
			if(!$scope.fileName) {
		    	...
		    	for(i in e.files) {
		    		...
		    		if(typeof e.files[i] == 'object'){
		    			if( e.files[i].size >= 3 * 1024 *1024 ) {
		    				$('#file_tip').text('文件过大！限制3M').addClass('ico-invalid');
		    				flag = false;
		    				return false;
		    			}
		    			files.push(e.files[i]);
...
define("js/controller/jobinfo", function(){});
